...
def check_if_valid_tile(tile, ai=None):
    ...
    if tile.has_ai:
        if tile.ai == ai:  # The AI already owns this tile (can't expand here)
            return False
        if ai is not None and not are_ais_at_war(ai, tile.ai):  # Friendly AI, don't expand into their territory
            return False
